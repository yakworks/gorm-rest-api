...
import gorm.tools.testing.BuildExampleData
...
class ProjectControllerSpec extends GebSpec implements RestApiTestTrait{
    ...
    BuildExampleData bed = new BuildExampleData(Project)

    //data to force a post or patch failure
    Map getInvalidData() { [num: null] }

    //Override if you don't want to use the autogenerated Example data.
    Map getInsertData() { println bed.buildValues(); bed.buildValues() }

    Map getUpdateData() {
        Map data = bed.buildValues()
        data.num = "test"
        data
    }
    ...
    void test_save_post() {
        ...
        //response.json.id
        subsetEquals(data, response.json, excludes)
        //Project.count() > 1// == 1
        def rget = restBuilder.get("$resourcePath/${response.json.id}")
        subsetEquals(data, rget.json, excludes)
    }
